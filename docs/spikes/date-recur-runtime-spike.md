# Date Recur runtime spike — adoption evidence

Disposition: `ADOPTED_BY_PROJECT_LEAD`

Authority: Epic #32 / Task #33 / Decision 0004.

## Project Lead Stage-A arbitration

The Project Lead adopted `date_recur` from the exact Stage-A candidate:

```text
TASK = #33
PR = #34
HEAD = f2f05fd41d5bbcd7d8c9d9ac76e123f54e3f4d7f
TREE = 46b0712829051e3e3788dca75f961cd2b8217b9d
DATE_RECUR_DECISION = ADOPT_DATE_RECUR
```

Stage B materializes that arbitration in Decision 0004 and isolates the
write-heavy synthetic spike harness from generic production verification. It
does not implement Personal Secretary domain entities/services or finalize an
exception persistence schema.
...
The adopted primitive is a production Composer dependency. No alternative
...
Decision 0004 now also requires Personal Secretary callers to bound every
operational expansion of an infinite recurrence. The upstream fail-safe is
defense-in-depth, not a substitute for that caller invariant.
...
entities. Decision 0004 therefore preserves ordinary occurrences as calculated
by default.

## Known upstream occurrence-table cleanup limitation

The live upstream Date Recur issue list contains the active `3.9.x-dev` bug:

```text
Occurrence tables are not cleared when corresponding field data is deleted
STATUS = Active
CATEGORY = Bug report
VERSION = 3.9.x-dev
```

This limitation is not an adoption blocker and Task #33 does not patch, fork or
apply a Composer patch to `date_recur`. No production-domain cleanup workaround
is introduced here.

Architectural consequence:

```text
technical occurrence-table cleanup semantics
MUST NOT
identify Personal Secretary domain truth or exception lifecycle
```

Application logic must never infer the business existence/deletion of an
occurrence or exception solely from those technical rows. A future upstream fix
may change cleanup behavior without changing this domain boundary.
...
This is an evidence-based semantic context requirement, not a final persistence
...
Future `ActivitySeries` design must explicitly address:

```text
audited revision/version
effective_from semantics or justified equivalent
auditable semantic series edits
orphaned exception detection/status
explicit reconciliation policy
```

Those mechanisms are constraints for a later bounded domain Task and are not
implemented here.
...
Stage B isolates the synthetic harness from generic runtime verification:

```text
DATE_RECUR_HARNESS = EXPLICIT_CI_ONLY
scripts/verify development|production = normal state/config/runtime verification
GENERIC_PRODUCTION_VERIFY_RUNS_SPIKE_FIXTURE = NO
```

The durable read-only `.github/workflows/drupal.yml` executes
`scripts/date-recur-spike.php` explicitly after development bootstrap and generic
verification. Development rebuilds and the production `--no-dev` rebuild then
exercise generic `scripts/verify` without creating/deleting the synthetic Date
Recur fixture. The workflow retains `permissions: contents: read`.

The temporary repository-write materialization workflow is not part of the
candidate and must not reappear.
...
Decision 0004 now materializes the Project Lead adoption. Task #33 still returns
to Project Lead for fresh exact-head review before any merge or domain work.
