...
/**
 * Authorizes current-user actions on one exact unmodified occurrence.
 */
final class CurrentUserOccurrenceAuthorityService {
  ...
  /**
   * Authorizes the current account for one exact current base occurrence.
   *
   * @return array{
  ...
  public function authorize(int $seriesId, string $originalOccurrenceKey): array {
    ...
    try {
      ...
      if ($householdId <= 0) {
        throw new InvalidArgumentException('Occurrence target has no valid Household.');
      }

      $authorizedIds = $this->householdAuthorization->authorizedHouseholdIds($user);
      if (!in_array($householdId, $authorizedIds, TRUE)) {
        throw new InvalidArgumentException('Occurrence target Household is not authorized.');
      }
      ...
      throw new InvalidArgumentException(
        'The requested occurrence is not currently authorized for this account.',
        0,
        $exception,
      );
    }
  ...
  private function currentPersistedUser(): UserInterface {
    ...
    if ($this->currentUser->isAnonymous() || $uid <= 0) {
      throw new InvalidArgumentException('Occurrence action requires an authenticated Drupal User.');
    }
    ...
    if (!$user instanceof UserInterface || !$user->isActive()) {
      throw new InvalidArgumentException('Occurrence action requires an active persisted Drupal User.');
    }
    if (
      ...
      throw new InvalidArgumentException('Occurrence action requires Personal Secretary product access.');
    }
  ...
  private function freshCurrentPerson(UserInterface $user): Person {
    try {
      ...
      throw new InvalidArgumentException('Occurrence action requires a valid Current Person.', 0, $exception);
    }

    $personId = (int) $person->id();
    if ($personId <= 0) {
      throw new InvalidArgumentException('Occurrence action requires a persisted Current Person.');
    }
    ...
    if (!$persisted instanceof Person || $persisted->isNew() || $persisted->id() === NULL) {
      throw new InvalidArgumentException('Occurrence action requires a current persisted Person.');
    }
  ...
  private function freshHousehold(int $householdId): Household {
    ...
    if (!$household instanceof Household || $household->isNew() || $household->id() === NULL) {
      throw new InvalidArgumentException('Occurrence target Household is unavailable.');
    }
    return $household;
  }
  ...
  private function exactEffectiveOccurrence(
    ...
    if (count($matches) !== 1) {
      throw new InvalidArgumentException('Occurrence target is no longer one exact effective base occurrence.');
    }
